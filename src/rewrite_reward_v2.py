...
4. **Collaboration with Other Actions**: Analyze how well the target actions collaborate with the actions of other agents. If they are executing the same actions, assign a score of 0; if they are effectively collaborating on different actions, assign a score of 1."""            prompt += "Please output the result in JSON format, structured as follows:"
    ...
    def generate_module_b_template(self, current_index, episode_experiences, evaluation):
        ...
        try:
            ...
            prompt += f"Evaluation for each actions:{evaluation}"
            ...
            # prompt += "Reward Value Distribution RequirementsThe reward difference between adjacent actions should be between 0.5 and 1.5.The ratio of positive to negative rewards should be approximately 3:7 to 4:6.It should resemble a normal distribution (bell curve)."
            prompt += f"Please output the result in JSON format, structured as follows, n = {self.context_window_size}:"
            prompt += "{'Inference Part': '...','Rewritten Reward Value': [X_1,X_2,X_3,...,X_n],'Reason for Rewriting': '...'}"
            
            # 组合输入模板
            module_b_template =  context_info + ""  + prompt
        ...
        try:
            ...
            for i in range(self.half_window,len(episode_experiences),self.context_window_size):
                ...
                while evaluation is None:
                    ...
                    if self.debug:
                        print(f"###################模块A的提示词模板：###################{module_a_template}")
                    ...
                    if module_a_result:
                        if self.debug:
                            print(f"###################模块A的输出：###################{json.dumps(module_a_result, indent=2, ensure_ascii=False)}")
                ...
                if self.debug:
                    print(f"###################模块B的提示词模板：###################{module_b_template}")
                ...
                while new_reward is None:
                    module_b_result = None
                    json_str = self.wrap_llm_response(module_b_template, is_module_a=False)
                    # 将JSON字符串转换为字典
                    json_str = clean_json_output(json_str)
                    print("json_str:\n",json_str)
                    json_data_list = extract_last_json(json_str)
                    for json_data in json_data_list:
                        if json_data is not None and "Rewritten Reward Value" in json_data:
                            module_b_result = json_data
            
                    if module_b_result:
                        if self.debug:
                            print(f"###################模块B的输出：###################{json.dumps(module_b_result, indent=2, ensure_ascii=False)}")
...
if __name__ == "__main__":
    ...
    # 保存数据到pickle文件
    save_file = './all_scenes_data_v4_1.pkl'
    with open(save_file, 'wb') as f:
        pickle.dump(all_scenes_data, f)

    print(f"数据保存完成{save_file}，耗时：{time.time() - start_time}秒")