...
def create_video_from_images(image_folder,_index = 0):
    # 获取所有png图片路径
    image_paths = [os.path.join(image_folder, f) for f in os.listdir(image_folder) if f.endswith('_map.png')]
    ...
    video_path = os.path.join(os.path.dirname(image_folder), f"mapping_20fps.mp4")
...
image_folder = f"{k_ex_result_folder}/{ex_index}/Images/0"
create_video_from_images(image_folder,0)
